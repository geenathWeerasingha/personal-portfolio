            ...
            <div className="relative pl-8 border-l-2 border-slate-700">
              {experiences.map((exp, index) => (
                <div key={index} className="relative mb-10 last:mb-0">
                  {/* Timeline dot */}
                  <div className="absolute -left-[25px] w-4 h-4 bg-cyan-500 rounded-full border-4 border-slate-950" />
                    ...
                    {exp.skills && exp.skills.length > 0 && (
                      <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-700/50">
                        {exp.skills.map((skill) => (
                          <span key={skill} className="text-xs px-2 py-1 bg-slate-800 rounded text-slate-400">
                            {skill}
                          </span>
                        ))}
                      </div>
              ...
              {education.map((edu, index) => (
                <div key={index} className="glass-card p-6">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    {edu.grade && (
                      <span className="text-slate-400 font-medium">{edu.grade}</span>
                    )}
                  </div>
                </div>
              ))}
