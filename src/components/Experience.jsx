        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            <div className="flex items-center gap-3 mb-8">
              <div className="w-10 h-10 bg-cyan-500/10 dark:bg-cyan-400/20 rounded-lg flex items-center justify-center">
              ...
              <h3 className="text-xl font-semibold text-slate-800 dark:text-white">Work Experience</h3>
              <span className="text-sm text-slate-500 dark:text-slate-500">1 yr 2 mos total</span>
            </div>
            ...
            <div className="relative pl-8 border-l-2 border-slate-200 dark:border-slate-700">
              {experiences.map((exp, index) => (
                <div key={index} className="relative mb-10 last:mb-0">
                  {/* Timeline dot */}
                  <div className="absolute -left-[25px] w-4 h-4 bg-cyan-500 dark:bg-cyan-400 rounded-full border-4 border-white dark:border-slate-950" />
                  ...
                  <div className="glass-card p-6">
                    <div className="flex flex-wrap items-start justify-between gap-2 mb-3">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-slate-400 dark:text-slate-600">{exp.period}</p>
                      </div>
                    </div>
                    
                    <p className="text-sm text-slate-500 dark:text-slate-500 mb-4">{exp.location}</p>
                      ...
                      <ul className="space-y-2 mb-4">
                        {exp.highlights.map((highlight, i) => (
                          <li key={i} className="flex items-start gap-2 text-sm text-slate-600 dark:text-slate-400">
                            <span className="text-cyan-500 dark:text-cyan-400 mt-1.5">•</span>
                            <span>{highlight}</span>
                          </li>
                        ))}
                      </ul>
                    ...
                    {exp.skills && exp.skills.length > 0 && (
                      <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-200 dark:border-slate-700">
                        {exp.skills.map((skill) => (
                          <span key={skill} className="text-xs px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded text-slate-600 dark:text-slate-400">
                            {skill}
                          </span>
                        ))}
                      </div>
          ...
          >
            <div className="flex items-center gap-3 mb-8">
              <div className="w-10 h-10 bg-teal-500/10 dark:bg-teal-400/20 rounded-lg flex items-center justify-center">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {education.map((edu, index) => (
                <div key={index} className="glass-card p-6">
                  <h4 className="text-lg font-semibold text-slate-800 dark:text-white mb-1">{edu.degree}</h4>
                  <p className="text-teal-600 dark:text-teal-400 font-medium mb-2">{edu.institution}</p>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-slate-500 dark:text-slate-500">{edu.duration}</span>
                    {edu.grade && (
                      <span className="text-slate-600 dark:text-slate-400 font-medium">{edu.grade}</span>
                    )}
                  </div>
                </div>
              ))}
